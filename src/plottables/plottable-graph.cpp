...
void QCPGraph::getPlotData(QVector<QPointF> *lineData, QVector<QCPData> *scatterData) const
{
  ...
  if (mKeyAxis->rangeReversed() != (mKeyAxis->orientation() == Qt::Vertical)) // make sure key pixels are sorted ascending in lineData and scatterData (significantly simplifies following processing)
  {
    if (lineData)
      std::reverse(lineData->begin(), lineData->end());
    if (scatterData)
      std::reverse(scatterData->begin(), scatterData->end());
  }
}
...
void QCPGraph::drawFill(QCPPainter *painter, QVector<QPointF> *lineData) const
{
  ...
  QVector<DataRange> segments = getNonNanSegments(lineData, keyAxis()->orientation());
  if (!mChannelFillGraph)
  {
    // draw base fill under graph, fill goes all the way to the zero-value-line:
    for (int i=0; i<segments.size(); ++i)
      painter->drawPolygon(getFillPolygon(lineData, segments.at(i)));
  } else
  {
    ...
    {
      QVector<DataRange> otherSegments = getNonNanSegments(&otherData, mChannelFillGraph->keyAxis()->orientation());
      QVector<QPair<DataRange, DataRange> > segmentPairs = getOverlappingSegments(segments, lineData, otherSegments, &otherData);
      for (int i=0; i<segmentPairs.size(); ++i)
        painter->drawPolygon(getChannelFillPolygon(lineData, segmentPairs.at(i).first, &otherData, segmentPairs.at(i).second));
    }
  }
}
...
/*! \internal
  ...
  The data points of this graph are passed as pixel coordinates via \a lineData, the data of the
